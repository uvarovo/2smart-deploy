...
class BridgesManager extends Base {
    ...
    async init() {
        ...
        const coreContainerName = process.env.CORE_CONTAINER_NAME || "2smart-core"; const { Config, NetworkSettings } = await docker.getContainer(coreContainerName).inspect();
        ...
        // RECOVERY: recreate Homie entities for bridges that lost them on restart
        this.debug.info("BridgesManager.init", "recover entities from configs");

        for (const id in this.bridges) {
            if (this.bridges[id].entity) continue;
            const bridgeType = this.bridges[id].type;
            if (!bridgeType) continue;

            const containerName = this.getContainerName(bridgeType, id);
            const configDir = path.join(PATH_TO_BRIDGE_CONFIGS, containerName);
            const envPath = path.join(configDir, ".env");

            if (!fs.existsSync(envPath)) continue;

            try {
                const envData = envfile.parseFileSync(envPath);
                let containerRunning = this.bridges[id].containerInfo && this.bridges[id].containerInfo.State === "running";
                // Build configuration from all fields defined in bridge type
                const btConfig = this.core.services.bridgeTypesManager.bridgeTypes[bridgeType];
                const btFields = (btConfig && btConfig.config && btConfig.config.configuration && btConfig.config.configuration.fields) || [];
                const recoveredConf = { DEVICE_NAME: envData.DEVICE_NAME || bridgeType, entityType: bridgeType };

                for (const { name } of btFields) {
                    if (name !== 'DEVICE_NAME' && envData[name] !== undefined && envData[name] !== '') {
                        recoveredConf[name] = envData[name];
                    }
                }
                const entityData = {
                    id: id,
                    type: bridgeType,
                    state: containerRunning ? "started" : "stopped",
                    configuration: recoveredConf
                };

                this.debug.info("BridgesManager.init.recoverEntity", { id: id, type: bridgeType });
                await this.core.homieMigrator.attachEntity(BridgeEntityType, entityData);
            } catch (e) {
                this.debug.warning("BridgesManager.init.recoverEntity", e);
            }
        }
        ...
        this.core.homie.subscribe('bridges/#');
    ...
    async attachNewBridgeEntity(entity) {
        ...
        const setConfiguration = async (configuration) => {
            // Preserve top-level subkeys of JSON-typed fields that the incoming
            // payload doesn't include. The admin UI renders only a subset of
            // each JSON blob (e.g. for KNX it edits nodes.config.extensions
            // but never touches nodes.config.deviceConfig). If we just write
            // whatever the UI sent, deviceConfig is wiped and the bridge
            // crashes on next start ("Cannot read property 'nodes' of
            // undefined"). Merge missing top-level keys back in from disk
            // before validating/saving/publishing.
            await this._preserveMissingJsonFieldKeys(entity, configuration);
    ...
    async _preserveMissingJsonFieldKeys(entity, configuration) {
        if (!configuration || typeof configuration !== 'object') return;
        let bridgeType;

        try {
            bridgeType = await this.retrieveBridgeType(entity.type);
        } catch (e) {
            this.debug.warning('BridgesManager._preserveMissingJsonFieldKeys', e);
        ...
        for (const field of bridgeType.configuration.fields) {
            if (field.type !== 'json' && field.type !== 'modbus-config') continue;
            const newVal = configuration[field.name];

            if (!newVal || typeof newVal !== 'object' || Array.isArray(newVal)) continue;
            const filePath = path.join(dir, `${field.name}.json`);

            if (!fs.existsSync(filePath)) continue;
            let oldVal;

            try {
                oldVal = JSON.parse(fs.readFileSync(filePath, 'utf8'));
            } catch (e) {
                this.debug.warning('BridgesManager._preserveMissingJsonFieldKeys', `failed to read ${filePath}: ${e.message}`);
                continue;
            }
            if (!oldVal || typeof oldVal !== 'object' || Array.isArray(oldVal)) continue;
            for (const key of Object.keys(oldVal)) {
                if (!(key in newVal)) {
                    newVal[key] = oldVal[key];
                    this.debug.warning('BridgesManager._preserveMissingJsonFieldKeys', `bridge ${entity.id}: '${field.name}.${key}' missing in incoming configuration; preserving existing value from disk to avoid data loss`);
                }
            }
            this._preserveReferencedListEntries(entity, field.name, newVal, oldVal);
        }
    }

    // The admin UI sometimes drops a node from `deviceConfig.nodes` while
    // still leaving its mapping entries in `extensions.mapping` (e.g. user
    // edited mapping but the UI's view of the node list was stale). The
    // bridge then refuses to start: app.js looks up each mapping key's node
    // id in `deviceConfig.nodes` and throws "Cannot find node with id X".
    // Restore any node from disk whose `id` is missing in the incoming
    // `deviceConfig.nodes` but still referenced by mapping/mappingPing keys.
    _preserveReferencedListEntries(entity, fieldName, newVal, oldVal) {
        try {
            const newList = newVal && newVal.deviceConfig && Array.isArray(newVal.deviceConfig.nodes)
                ? newVal.deviceConfig.nodes : null;
            const oldList = oldVal && oldVal.deviceConfig && Array.isArray(oldVal.deviceConfig.nodes)
                ? oldVal.deviceConfig.nodes : null;
            ...
            const collect = (obj) => {
                ...
                for (const key of Object.keys(obj)) {
                    ...
                    referenced.add(dot >= 0 ? k.slice(0, dot) : k);
                }
            };
            const ext = newVal.extensions;

            if (ext && typeof ext === 'object') {
                collect(ext.mapping);
                collect(ext.mappingPing);
            }
            // Mapping keys are always strings, so coerce node ids to string for
            // membership tests; otherwise numeric ids would never match the
            // string prefixes parsed out of the keys.
            const newIds = new Set(newList.map((n) => n && n.id).filter((id) => id != null).map(String));

            for (const oldNode of oldList) {
                if (!oldNode || typeof oldNode !== 'object' || oldNode.id == null) continue;
                const oldId = String(oldNode.id);

                if (newIds.has(oldId)) continue;
                if (!referenced.has(oldId)) continue;
                newList.push(oldNode);
                newIds.add(oldId);
                this.debug.warning('BridgesManager._preserveReferencedListEntries', `bridge ${entity.id}: '${fieldName}.deviceConfig.nodes[id=${oldNode.id}]' missing in incoming configuration but still referenced by extensions.mapping; restoring from disk to avoid bridge start crash`);
            }

            // Still-missing referenced prefixes are brand-new nodes the
            // admin UI invented in extensions.mapping but never added to
            // deviceConfig.nodes. Synthesize a stub by cloning the most
            // structurally similar existing node, or building from mapping
            // sensor ids if no similar node exists. Without this, the
            // bridge crashes on start with "Cannot find node with id <X>"
            // every time the user adds a new device through the UI.
            const stillMissing = [];

            for (const ref of referenced) {
                if (!newIds.has(ref)) stillMissing.push(ref);
            }
            if (!stillMissing.length) return;
            const mapping = ext && ext.mapping && typeof ext.mapping === 'object' ? ext.mapping : {};
            ...
            for (const ref of stillMissing) {
                ...
                for (const [k, v] of Object.entries(mapping)) {
                    ...
                    if (slash < 0) continue;
                    if (k.slice(0, slash) !== ref) continue;
                    const sid = k.slice(slash + 1);

                    sensorIds.add(sid);
                    sensorMeta[sid] = v && v.transport ? v.transport : {};
                }
                let stub = null;
                let bestOverlap = 0;

                for (const candidate of newList) {
                    if (!candidate || !Array.isArray(candidate.sensors)) continue;
                    const candSensorIds = candidate.sensors.map((s) => s && s.id).filter((x) => x != null);
                    const overlap = candSensorIds.filter((id) => sensorIds.has(id)).length;
                ...
                if (stub && bestOverlap >= Math.max(1, Math.floor(sensorIds.size / 2))) {
                    synthesized = JSON.parse(JSON.stringify(stub));
                    synthesized.id = ref;
                    synthesized.name = ref;
                } else {
                    synthesized = {
                        id: ref,
                        name: ref,
                        sensors: [...sensorIds].map((sid) => {
                            const t = sensorMeta[sid] || {};
                            const dpt = typeof t.dpt === 'string' ? t.dpt : '';
                            const isInt = /^DPT[157]\./i.test(dpt) || /^DPT(20|232)\./i.test(dpt);

                            return {
                                id: sid,
                                name: sid,
                                dataType: isInt ? 'integer' : 'float',
                                retained: true,
                                settable: typeof t.set === 'string' && t.set.length > 0,
                            };
                        }),
                        options: [],
                        telemetry: [],
                ...
                newList.push(synthesized);
                newIds.add(ref);
                this.debug.warning('BridgesManager._preserveReferencedListEntries', `bridge ${entity.id}: '${fieldName}.deviceConfig.nodes[id=${ref}]' referenced by extensions.mapping but missing from incoming configuration AND from disk; synthesized stub${stub ? ` by cloning '${stub.id}' (overlap=${bestOverlap}/${sensorIds.size})` : ` from mapping (${sensorIds.size} sensors)`} to avoid bridge start crash`);
            }
        } catch (e) {
            this.debug.warning('BridgesManager._preserveReferencedListEntries', e);
        }
    }
